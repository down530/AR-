...
# Lazy import rembg — only loads on first use
_rembg = None
_rembg_failed = False


def _get_rembg():
    """Lazy-load rembg with model download failure handling."""
    global _rembg, _rembg_failed
    if _rembg_failed:
        return None
    if _rembg is None:
        try:
            ...
            _rembg_failed = True
            logger.warning(f"rembg not available: {e}. Background removal will be skipped.")
            return None
    return _rembg
...
def remove_background(img_path: Path) -> Image.Image:
    ...
    Falls back to returning the original image if rembg is unavailable.
    ...
    with Image.open(img_path).convert("RGBA") as im:
        if remove_fn is not None:
            ...
            logger.info("Skipping background removal — rembg model not available.")
            return im
