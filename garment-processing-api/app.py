...
# Explicitly load .env from the project root (uvicorn reloader subprocess may have different CWD)
...
from config import (
    ...
    USE_CLOUDINARY, LOCAL_STORAGE_DIR, LOCAL_ORIG_DIR, LOCAL_CUT_DIR, LOCAL_TRYON_DIR, LOCAL_BASE_URL
)
...
LOCAL_ORIG_DIR.mkdir(parents=True, exist_ok=True)
LOCAL_CUT_DIR.mkdir(parents=True, exist_ok=True)
LOCAL_TRYON_DIR.mkdir(parents=True, exist_ok=True)

# Mount local storage for serving files
app.mount("/static", StaticFiles(directory=str(LOCAL_STORAGE_DIR)), name="static")
...
def _store_image(data: bytes, public_id: str, folder: str, fmt: str = "png") -> dict:
    """Save locally or upload to Cloudinary. Returns {'secure_url', 'public_id'}."""
    if USE_CLOUDINARY:
        folder_map = {"orig": FOLDER_ORIG, "cut": FOLDER_CUT, "tryon": FOLDER_TRYON}
        result = upload_bytes(data, public_id, folder_map[folder], fmt if fmt else None)
        return {"secure_url": result["secure_url"], "public_id": result["public_id"]}
    dir_map = {"orig": LOCAL_ORIG_DIR, "cut": LOCAL_CUT_DIR, "tryon": LOCAL_TRYON_DIR}
    out_dir = dir_map[folder]
    fname = f"{public_id}.{fmt}" if fmt else public_id
    (out_dir / fname).write_bytes(data)
    url = f"{LOCAL_BASE_URL}/static/{out_dir.name}/{fname}"
    return {"secure_url": url, "public_id": public_id}
...
async def classify_garment(request: Request, garment: UploadFile = File(...)):
    ...
    # Store original (Cloudinary or local)
    token = secrets.token_hex(8)
    orig_public_id = f"garment_{token}"

    try:
        orig_upload = await run_in_threadpool(_store_image, body, orig_public_id, "orig", "")
    ...
    try:
        ...
        # Store cutout (Cloudinary or local)
        cutout_public_id = f"cutout_{token}"
        cutout_upload = await run_in_threadpool(_store_image, cutout_bytes, cutout_public_id, "cut", "png")
...
async def detect_garment_type(request: Request, garment: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"[{request_id}] Classification fell back to UNKNOWN: {e}")
            label, conf = "UNKNOWN", 0.0
...
async def construct_outfit(
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"[{request_id}] Upper garment classification fell back: {e}")
            upper_label, upper_conf = "UNKNOWN", 0.0
        ...
        try:
            ...
            logger.warning(f"[{request_id}] Lower garment classification fell back: {e}")
            lower_label, lower_conf = "UNKNOWN", 0.0
        ...
        logger.info(f"[{request_id}] Storing individual garments...")

        # Store upper garment
        upper_public_id = f"upper_{token}"
        upper_upload = await run_in_threadpool(_store_image, upper_body, upper_public_id, "orig", "")
        upper_url = upper_upload["secure_url"]

        # Store lower garment
        lower_public_id = f"lower_{token}"
        lower_upload = await run_in_threadpool(_store_image, lower_body, lower_public_id, "orig", "")
        ...
        upper_cutout_upload = await run_in_threadpool(_store_image, upper_cutout_bytes, upper_cutout_public_id, "cut", "png")
        ...
        lower_cutout_upload = await run_in_threadpool(_store_image, lower_cutout_bytes, lower_cutout_public_id, "cut", "png")
        ...
        # Store constructed outfit
        outfit_public_id = f"outfit_{token}"
        outfit_upload = await run_in_threadpool(_store_image, outfit_bytes, outfit_public_id, "orig", "png")
...
async def virtual_tryon(
    ...
    # Store person image (Cloudinary or local)
    token = secrets.token_hex(8)
    person_public_id = f"person_{token}"

    try:
        person_upload = await run_in_threadpool(_store_image, person_body, person_public_id, "orig", "")
        person_url = person_upload["secure_url"]
        logger.info(f"Person image stored: {person_url}")
    except Exception as e:
    ...
    if process_garment:
        ...
        try:
            # Store original garment
            garment_public_id = f"garment_{token}"
            garment_upload = await run_in_threadpool(_store_image, garment_body, garment_public_id, "orig", "")
    ...
    try:
        ...
        result_upload = await run_in_threadpool(_store_image, result_bytes, result_public_id, "tryon", "png")
        result_url = result_upload["secure_url"]
        logger.info(f"Try-on result stored: {result_url}")
