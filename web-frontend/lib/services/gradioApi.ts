...
/**
 * Perform virtual try-on via Gradio client (direct to HF Space).
 * This bypasses the backend entirely — no Cloudinary, no classification needed.
 */
export async function tryOnViaGradio(
  personFile: File,
  clothFile: File,
  ...
  seed: number = 42,
  showType: 'result only' | 'input & result' | 'input & mask & result' = 'input & result',
  signal?: AbortSignal,
): Promise<string> {
  console.log('🚀 Gradio Client Try-On Request:', {
    personImage: personFile.name,
    clothImage: clothFile.name,
    clothType,
    numInferenceSteps,
    guidanceScale,
    seed,
  ...
  // Connect to HF Space (caches connection)
  const client = await Client.connect(GRADIO_SPACE, HF_TOKEN ? { hf_token: HF_TOKEN } : undefined);

  const startTime = Date.now();

  // Call submit_function via Gradio API
  // The Gradio Space's submit_function expects:
  //   person_image: ImageEditor data {background, layers, composite}
  //   cloth_image: File/path
  //   cloth_type: "upper" | "lower" | "overall"
  //   num_inference_steps, guidance_scale, seed, show_type
  let result: any;
  try {
    result = await withTimeout(
      client.predict('/submit_function_p2p', {
        person_image: {
    ...
    console.error('❌ Gradio predict failed:', predictErr?.message || predictErr);
    // Log the full error for debugging
    if (predictErr?.response) {
      console.error('❌ Response data:', predictErr.response);
    }
    throw new Error(predictErr?.message || 'Gradio prediction failed. The HF Space may be sleeping or unavailable. Please try again in 30 seconds.');
  }

  const duration = Date.now() - startTime;
  console.log('✅ Gradio Client Try-On Success:', {
    duration: `${(duration / 1000).toFixed(2)}s`,
    resultType: typeof result,
    resultKeys: result ? Object.keys(result) : null,
  });

  // Handle result — Gradio client returns {type:"data", data:[...], endpoint, fn_index}
  let dataUrl: string | null = null;

  // Helper: extract FileData URL/path from any object
  const extractFileData = (obj: any): string | null => {
    if (!obj || typeof obj !== 'object') return null;
    // Gradio FileData: {url, path, orig_name, meta: {_type: "gradio.FileData"}}
    if (obj.url) return obj.url;
    if (obj.path) return obj.path;
    // Nested _value
    if (obj._value) {
      if (typeof obj._value === 'string') return obj._value;
      if (obj._value?.url) return obj._value.url;
      if (obj._value?.path) return obj._value.path;
    }
    return null;
  };

  if (result && typeof result === 'object') {
    // Case 1: Gradio client v1.x format: {type:"data", data:[FileData, ...]}
    if (result.type === 'data' && Array.isArray(result.data)) {
      console.log('📦 Gradio data result, items:', result.data.length);
      for (const item of result.data) {
        const url = extractFileData(item);
        if (url) {
          dataUrl = await resolveGradioFileToDataURL(url);
          break;
        }
      }
    }
    // Case 2: Direct FileData object {url, path, orig_name}
    else if (result.url || result.path) {
      dataUrl = await resolveGradioFileToDataURL(extractFileData(result)!);
    }
    // Case 3: result.data is a single FileData (not array)
    else if (result.data && typeof result.data === 'object') {
      const url = extractFileData(result.data);
      if (url) dataUrl = await resolveGradioFileToDataURL(url);
    }
    // Case 4: Legacy _value wrapper
    else if (result._value) {
      const url = extractFileData(result);
      if (url) dataUrl = await resolveGradioFileToDataURL(url);
    }
    // Case 5: It's an array directly
      ...
      for (const item of result) {
        ...
        if (typeof item === 'string') {
          dataUrl = await resolveGradioFileToDataURL(item);
          break;
        }
      }
    }
    ...
    // Direct string URL
    dataUrl = await resolveGradioFileToDataURL(result);
  }

  if (!dataUrl) {
    console.error('❌ Could not extract image from Gradio result:', JSON.stringify(result)?.slice(0, 500));
    throw new Error('Failed to extract try-on result from Gradio response. Check browser console for details.');
  }
...
/**
 * Resolve a Gradio file path/URL to a data URL.
 */
async function resolveGradioFileToDataURL(pathOrUrl: string): Promise<string> {
  const resolvedUrl = resolveGradioFileUrl(pathOrUrl);
  ...
  try {
    ...
    return await blobToDataURLSafe(blob);
  } catch (err) {
    console.error('❌ Failed to download/convert result from:', resolvedUrl, err);
    // Fallback: return the URL directly
    console.warn('⚠️ Returning raw Gradio URL as fallback');
    return resolvedUrl;
  }
}
