...
export async function detectGarmentType(
  ...
  try {
    const { data } = await garmentHttp.post<{
      label: string;
      confidence: number;
      filename: string;
      file_size_bytes: number;
    ...
    // If the local garment API is not running (CORS / connection refused),
    // fall back to the Railway deployment (production endpoint).
    // This avoids "Model not loaded" 500 errors when the local ML model
    // hasn't been loaded yet.
    if (GARMENT_API_BASE.startsWith('http://1') || GARMENT_API_BASE.startsWith('http://localhost')) {
      try {
        const railwayFallback = 'https://ar-fashion-tryon-production.up.railway.app';
        console.warn('🔄 Local garment API unavailable, falling back to Railway deployment...');
        const { data: fallbackData } = await http.post<{
          label: string;
          confidence: number;
          filename: string;
          file_size_bytes: number;
          ...
          `${railwayFallback}/detect_garment_type`,
          formData,
          {
            signal,
            headers: { Accept: 'application/json' },
        ...
        console.log('✅ Railway fallback detection success:', fallbackData.label);
        return fallbackData;
      } catch (fallbackErr: unknown) {
        console.error('❌ Railway fallback also failed:', fallbackErr);
      }
    }
