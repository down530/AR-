...
import { tryOnViaGradio } from '@/lib/services/gradioApi';
...
export const useVtonStore = create<VtonState>((set, get) => ({
  ...
  setBody: async (file) => {
    ...
    try {
      ...
      if (qualityCheck.level === 'POOR') {
        ...
        // Still show toast for poor quality (non-blocking warning)
        toast.warning('Body photo quality is low — results may be less accurate', { duration: 3000 });
      } else if (qualityCheck.level === 'OK') {
        console.log('ℹ️ Body photo quality is acceptable (not optimal):', qualityCheck.suggestions);
      }
    } catch (err) {
  ...
  tryOn: async () => {
    ...
    try {
      ...
      // Call Gradio client directly (bypasses backend/Cloudinary)
      const dataUrl = await tryOnViaGradio(
        body.file,
        garmentFileForTryOn,
        clothTypeForTryOn,
      ...
      console.log('✅ Virtual try-on complete:', {
        result_url: dataUrl.substring(0, 50) + '...',
        mode: tryOnPath,
      });

      set({ status: 'done', resultUrl: dataUrl, step: 'RESULT' });
    } catch (err: unknown) {
      const error = err as Error;
